   ...
   // Rigidbody rigidBody;
	...
	void Update () {
		...
		if (chCon.isGrounded||isGround) {
			...
			if(angles.x > 180 && angles.x < 340 && GamePad.GetAxis(GamePad.Axis.LeftStick,GamePad.Index.Three).y > 0)
			{
				angles = new Vector3(340, angles.y, angles.z);
			}
			else if(angles.x <= 180 && angles.x > 20 && GamePad.GetAxis(GamePad.Axis.LeftStick,GamePad.Index.Three).y <= 0)
			{
